...
/*   Updated: 2024/10/01 18:30:23 by emansoor         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_map_size(t_map *map, int fd)
{
	...
	while (status > -1 && line && ft_strcmp(line, "\n") != 0)
	{
		if (is_valid_row(line) && !has_spaces_only(line))
		{
			map->row_count++;
			if ((ft_strlen(line) - 1) > map->col_count)
				map->col_count = ft_strlen(line) - 1;
		}
		else
		{
			status = -2;
	...
	free(line);
	if (status == -2 || map->row_count < 3 || map->row_count > 400
		|| map->col_count < 3 || map->col_count > 400 || check_file_end(fd) > 0)
		return (1);
	return (0);
}
